...
所以最近修改了一个阿里云函数的版本。将data，cookies，ali.py上传至云函数平台，设置触发任务为时间触发 0 30 0 * * * 即可在八点半运行。若上传了cookies则不必在脚本中填写账号密码，否则必须填写（cookies本身有效期是1个月，如果本地用session每次会更新不会失效，在云函数上不确定，正在测试）
