...
class ZJUHealthReport():
    def __init__(self, user, passwd, ua, api, area, cwd):
        ...
        if os.path.exists(self.cwd+'cookies'):
            with open(self.cwd+'cookies', encoding='utf-8') as f:
                cookies = eval(f.read())
            self.session.cookies = requests.utils.cookiejar_from_dict(cookies, cookiejar=None, overwrite=True)
            #os.remove(self.cwd+'cookies')
        else:
    ...
    def DK(self):
        ...
        with open(self.cwd+'html','w', encoding='utf-8') as f:
            f.write(res.text)
        if re.search("hasFlag: '1'", res.text):
            print('已打卡')
            #with open(self.cwd+'cookies', 'w', encoding='utf-8') as f:
        ...
        with open(self.cwd+'cookies', 'w', encoding='utf-8') as f, open(self.cwd+'log', 'a') as log:
...
if __name__ == '__main__':
    ...
    api = r'https://oapi.dingtalk.com/robot/send?access_token='
    ...
    try:
        DK = ZJUHealthReport(usr, pwd, ua, api, area, cwd)
        DK.DK()
